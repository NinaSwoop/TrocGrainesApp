...
export default function AdDetails() {
    ...
    if (!ad) {
        return <p className="text-red-500 text-center mt-10">Aucune annonce trouvée.</p>;
    }
    ...
    return (
        <div className="container mx-auto px-4 py-10 pt-[7rem]">
            ...
            <div className="flex flex-col items-center mt-6">
                ...
                <p className="mt-2 text-sm lg:text-base text-green-dark">Lieu : {ad.location}</p>
                <p className="mt-2 text-sm lg:text-base text-green-dark">Catégorie : {ad.category}</p>
                {ad.adStatus === 'reserved' && (
                    <p className="text-red-500 font-semibold mt-4">Cette annonce est réservée</p>
                )}
                {user ? (
                    <Link to={"/messaging"}>
