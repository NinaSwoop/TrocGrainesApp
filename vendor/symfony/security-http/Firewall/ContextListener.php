    ...
    protected function refreshUser(TokenInterface $token): ?TokenInterface
    {
        ...
        foreach ($this->userProviders as $provider) {
            if (!$provider instanceof UserProviderInterface) {
                throw new \InvalidArgumentException(\sprintf('SymfonyUser provider "%s" must implement "%s".', get_debug_type($provider), UserProviderInterface::class));
            }
            ...
            try {
                ...
                if (null !== $this->logger) {
                    ...
                    $this->logger->debug('SymfonyUser was reloaded from a user provider.', $context);
                }
